...
- **Forms**: `ReactContactForm.tsx` submits to Netlify Forms (`form-name` hidden field, `netlify-honeypot` bot field) via a `fetch` POST to `/` with `application/x-www-form-urlencoded` — this only works correctly when served through Netlify (hence `npm run preview` uses `netlify dev`, not plain `astro preview`).
